...
def run(i):
    ...
    model.add(Conv1D(8 ,kernel_size=12, strides=1,activation='relu',
    ...
    model.add(Conv1D(4, kernel_size=8, strides=1,activation='relu'))
    # Then use the MaxPooling2D function to add a 2D max pooling layer, with pooling filter sized 2X2 and stride of 2 in x and y directions.

    model.add(MaxPooling1D(pool_size=1, strides=1))

    model.add(Conv1D(2, kernel_size=2, strides=1, activation='relu'))
    ...
    if (test1_corr_>0.823) & (test2_corr_>0.873):
        ...
        rmm1_05_15.to_csv('/home/cccr/supriyo/panini/filtered_data/historical/JGRJD/conv1d/ensamble3/rmm1_1905_2015_'+str(i)+'p1.txt')
        print(train_corr_,test1_corr_,test2_corr_)
        model.save("/home/cccr/supriyo/panini/filtered_data/historical/JGRJD/conv1d/ensamble3/model_rmm1_p1_"+str(i)+".h5")
        print("Saved rmm1 model to disk")
    return i 
