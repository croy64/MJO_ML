...
def data_hilbert(st,en):
   ...
   ## Read Data
    ...
    X2 = X.copy() ## filter 10-30 days
    for i in range(X2.shape[1]):
        signal = X2.iloc[:,i].values
        temp = filter_signal_scipy(signal,sample_freq=1,ltime_period=10,htime_period=30,keep_mean=1)
        X2.iloc[:,i] = np.real(temp)
   
    hilbertx_filt2       = pd.DataFrame(np.imag(hilbert(X2,axis=0)))
    hilbertx_filt2.index = X2.index
    
    
    
    X3 = X.copy()  ## filter 30 -90 days
    for i in range(X3.shape[1]):
        signal = X3.iloc[:,i].values
        temp = filter_signal_scipy(signal,sample_freq=1,ltime_period=30,htime_period=90,keep_mean=1)
        X3.iloc[:,i] = np.real(temp)
   
    hilbertx_filt3       = pd.DataFrame(np.imag(hilbert(X3,axis=0)))
    hilbertx_filt3.index = X3.index
...
#     #####################################
   ...
   #######################################################
    ...
    X4             = pd.concat([X1,hilbertx,X2.iloc[119:],hilbertx_filt2[119:],X3.iloc[119:],hilbertx_filt3[119:]],axis=1)
